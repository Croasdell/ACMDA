...
class ACMDATest extends TestCase
{
    ...
    public function testRegenerateMessageRedraftsAndResetsStatus(): void
    {
        $db = $this->getDb();
        $id = receiveMessage($db, 'dan', 'plumbing repairs');
        $db->exec("UPDATE wa_messages SET draft='old', status='rejected' WHERE id=$id");
        regenerateMessage($db, $id);
        $row = $db->query('SELECT draft, status FROM wa_messages WHERE id = ' . $id)->fetch(PDO::FETCH_ASSOC);
        $this->assertSame('pending', $row['status']);
        $this->assertStringContainsString('plumbing repairs', $row['draft']);
    }
